...
Tags generated by `getTags()` in `cli.cjs`: `version`, `major`, `major.minor`, and optionally `latest`. When the resolved version includes a prerelease suffix, the `major` and `major.minor` tags preserve that suffix (for example, `1.2.3-beta.1` becomes `1-beta.1` and `1.2-beta.1`). The full version string is also passed as `--build-arg APP_VERSION=<full>`.

## Alias tag system

Alias tags are generated in `getAliasComputation(version, settings, buildType)` and then appended by `getTagComputation()`.

There are two independent alias paths (both may be enabled together):

- Simple toggles: `docker.aliases.branch`, `docker.aliases.sanitizedBranch`
- Rule-based aliases: `docker.aliases.rules[]` (ordered, first-match-wins selection)

Global alias settings are resolved in `getDockerSettings()` as:

- `aliasPrefix`
- `aliasSuffix`
- `aliasMaxLength`
- `aliasNonPublicPrefix`

`aliasNonPublicPrefix` is opt-in and defaults to `""` (empty). If configured (for example `"np-"`), it is applied only when the computed build type is non-public.

Rule schema (`normalizeAliasRule`):

- `id`
- `match` (wildcard or `regex:` prefixed expression)
- `template`
- `alias` / `aliases`
- `sanitize` (`none` | `branch` | `sanitized`)
- `prefix`, `suffix`, `maxLength`, `nonPublicPrefix`
- `tagPrefix`, `tagSuffix`, `tagMaxLength`, `tagNonPublicPrefix`

A matching rule can either add alias tags or transform the generated semantic tags (or both). Tag transform fields rewrite the full semantic tag set when the rule is selected.

Template expansion (`expandAliasTemplate`) supports:

- `$BRANCH`
- `$BRANCH_SANITIZED`
- `$1..$9` capture substitution from regex matches

Plan output (`dock plan --json`) includes alias trace information under `plan.aliasPolicy` and `plan.tagComputation.aliases`.
