...
const PROVIDER_GITVERSION = "gitversion";
...
function mergeBuildConfig(config) {
  ...
  return {
    ...
    [VERSION_SECTION_KEY]: {
      ...
      [PROVIDER_GITVERSION]: {
        ...defaults[VERSION_SECTION_KEY][PROVIDER_GITVERSION],
        ...(version[PROVIDER_GITVERSION] || {})
      },
      [PROVIDER_NODEJS]: {
...
function repoHasGitVersion(repoRoot, buildConfig) {
  const gitVersionConfigFile = buildConfig?.[VERSION_SECTION_KEY]?.[PROVIDER_GITVERSION]?.configFile || "GitVersion.yml";
  return fileExists(resolvePath(repoRoot, gitVersionConfigFile));
}
...
function detectProviderName(repoRoot, buildConfig, env) {
  ...
  if (repoHasGitVersion(repoRoot, buildConfig)) {
    return PROVIDER_GITVERSION;
  }
...
function resolveVersion(context) {
  ...
  try {
    ...
    process.stderr.write(
      `${WARNING_PREFIX} Auto-detected version provider '${providerName}' failed; retrying with '${PROVIDER_ENV}'.\n`
    );
